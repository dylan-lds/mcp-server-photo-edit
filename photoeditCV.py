...
def draw_text_on_image(image_path: str, textLines: list[TextInfo], output_path=None):
    ...
    # 使用OpenCV的inpainting算法修复文字区域
    inpainted_image = cv2.inpaint(cv_image, mask, inpaintRadius=3, flags=cv2.INPAINT_TELEA)
...
def photo_edit_tool(imagePath: str, textLines: list[TextInfo]):
    ...
    Args:
        imagePath: 要编辑的图片路径。
        textLines: 文本行信息列表，每个文本行包含文本内容、边界框信息。
